...
public struct QETag {
    static func small(ch : Channel) -> Async<String> {
        return ch.readAt(0).bindRet(.Sync) { data in
            let bs = Bytes(data: [UInt8(0x16)])
                .concat(data.toBytes().sha1())
    ...
    static func big(ch : Channel, _ worker : Int) -> Async<String> {
        ...
        { (blockId : Int) -> Async<Bytes> in
            ...
            return ch.readAt(blockStart, blockSize)
            .bindRet(.Sync) { data in data.toBytes().sha1() }
        }
        
        return limitedParallel(worker, works).bindRet(.Sync)
        { bs -> String in
            let sha = Bytes.concat(bs).sha1()
            let acc = Bytes(data: [UInt8(0x96)]).concat(sha)
    ...
    public static func hash(
        ...
        return hash(
            ch: try! Channel(path: path, oflag: O_RDONLY),
            worker : worker
        )
    }
}